...
class CardAdvancedComponent extends BaseCardComponent {
  ...
  Future<void> _onSubmit(ComponentCommunicationModel event) async {
    try {
      ...
      final PaymentEventDTO paymentEventDTO = paymentEventHandler.mapToPaymentEventDTO(paymentEvent);
      ComponentPlatformApi.instance.onPaymentsResult(componentId, paymentEventDTO);
    } catch (exception) {
  ...
  Future<void> _onAdditionalDetails(ComponentCommunicationModel event) async {
    try {
      final PaymentEvent paymentEvent = await _getOnAdditionalDetailsPaymentEvent(event);
      final PaymentEventDTO paymentEventDTO = paymentEventHandler.mapToPaymentEventDTO(paymentEvent);
      ComponentPlatformApi.instance.onPaymentsDetailsResult(componentId, paymentEventDTO);
    } catch (exception) {
      ComponentPlatformApi.instance.onPaymentsResult(
        componentId,
        PaymentEventDTO(
  ...
  Future<PaymentEvent> _getOnSubmitPaymentEvent(ComponentCommunicationModel event) async {
  ...
  Future<PaymentEvent> _getOnAdditionalDetailsPaymentEvent(ComponentCommunicationModel event) async {
    switch (advancedCheckout) {
      case AdvancedCheckout it:
